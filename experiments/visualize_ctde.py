...
def load_trained_model(checkpoint_path, checkpoint_num=None):
    ...
    policies = {
        "shared_policy": (None, obs_space, act_space, shared_policy_config)
    }
...
def run_visualization(trainer, env_config, render=True, num_episodes=1, horizon=None):
    ...
    try:
                    ...
                    try:
                        ...
                        if env.lane_change_enabled:
                            ...
                            if isinstance(action, np.ndarray):
                                ...
                                elif action.shape == (1,) or len(action) == 1:
                                    action_dict[agent_id] = np.array([action[0], 0.0], dtype=np.float32)
                                else:
                                    action_dict[agent_id] = np.array(
                                        [float(action.flatten()[0]), 0.0], dtype=np.float32
                                    )
                            else:
                            ...
                            if isinstance(action, np.ndarray):
                                if action.shape == (1,) or len(action) == 1:
                                    ...
                                    action_dict[agent_id] = float(action.flatten()[0])
                            else:
        ...
        metrics_file = os.path.join(metrics_dir, f"ctde_metrics_{timestamp}.json")
        raw_data_file = os.path.join(metrics_dir, f"ctde_raw_data_{timestamp}.csv")
    ...
    try:
        ...
        if args.checkpoint_path is None:
            ...
            if checkpoint_path is None:
                print("ERROR: No checkpoints found in results/ctde/")
                print("Please train a model first using train_ctde.py")
                return
